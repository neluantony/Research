...
def _fetch_model(conn, name: str) -> tuple[int, str, str]:
    with conn.cursor() as cur:
        cur.execute("SELECT model_id, exact_version_string, family FROM models "
...
def cmd_run(args) -> int:
    ...
    with db.connect() as conn:
        if args.model == "mock":
            ...
            model_id, version_string, family = _fetch_model(conn, args.model)
            if family == "gemini":
                provider = (GeminiProvider(version_string, pause_s=args.pause)
                            if args.pause is not None else GeminiProvider(version_string))
            elif family == "claude":
                provider = AnthropicProvider(version_string)
            else:
                raise SystemExit(f"no provider implemented for family '{family}'")
        ...
        res = harness.run(conn, provider, model_id, prompt_row, args.task,
                          ...
                          scheme=args.scheme, per_city=args.per_city)
    if args.write:
...
def cmd_export_bundle(args) -> int:
    ...
    with db.connect() as conn:
        meta = bundle.export_bundle(conn, args.out, per_city=args.per_city,
                                    prompt_version=args.prompt_version)
    print(f"bundle written to {args.out}")
    print(f"  {meta['n_points']} points · scheme {meta['scheme']} · "
          f"prompt {meta['prompt_version']}")
    print("  contents: grids/, manifest.jsonl, prompt_city.txt, "
          "schema_city.json, bundle_meta.json")
    return 0
...
def cmd_import_bundle(args) -> int:
    ...
    with db.connect() as conn:
        res = bundle.import_results(conn, args.results, args.bundle)
    print(f"imported {res['imported']} responses for {res['model_name']} "
          f"(run_id {res['run_id']}, {res['skipped']} skipped)")
    print("next: python -m inference score --write")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    r.add_argument("--per-city", type=int,
                   help="balanced pilot subset: this many points per city "
                        "(split evenly across strata; same subset for every scheme/model)")
    ...
    r.add_argument("--pause", type=float,
                   help="seconds between requests (default: the provider's "
                        "free-tier pacing; use ~1 on the paid tier)")
    r.add_argument("--write", action="store_true", help="actually call the model + store responses")
    r.set_defaults(func=cmd_run)

    eb = sub.add_parser("export-bundle",
                        help="export a self-contained Colab inference bundle (grids + prompt + schema)")
    eb.add_argument("--out", default="colab/bundle_city_grid", help="output directory")
    eb.add_argument("--per-city", type=int, default=10,
                    help="balanced subset size per city (default 10 = the pilot's 124 points)")
    eb.add_argument("--prompt-version", default="city_grid_v1")
    eb.set_defaults(func=cmd_export_bundle)

    ib = sub.add_parser("import-bundle-results",
                        help="load a Colab responses JSONL into the DB")
    ib.add_argument("--results", required=True, help="responses_<model>.jsonl from Colab")
    ib.add_argument("--bundle", default="colab/bundle_city_grid",
                    help="the bundle dir the run used (for the manifest)")
    ib.set_defaults(func=cmd_import_bundle)
    return p
