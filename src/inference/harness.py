...
and resumed anytime, and answered points are skipped. Scoring happens
...
def _num(v) -> float | None:
    """Coerce to float, or None. Small open models sometimes put a string
    like 'unknown' in a coordinate field; that must not reach a numeric column."""
    try:
        return float(v)
...
def parse_city_fields(parsed: dict | None) -> dict:
    ...
    return {
        ...
        "pred_lat": _num(parsed.get("latitude")),
        "pred_lon": _num(parsed.get("longitude")),
...
def pending_points(conn, model_id: int, prompt_id: int, task: str,
                   ...
                   per_city: int | None = None) -> list[tuple]:
    ...
    per_city takes a balanced pilot subset: per_city/5 points per stratum
    per city, ranked by point_id over ALL imaged points, and the rank ignores
    what has been answered, so the subset is identical across schemes and
    models and the two conditions compare like with like."""
    nbhd_filter = (
    ...
    params: list = [scheme, model_id, prompt_id, scheme]
    pilot_filter = ""
    if per_city:
    ...
    with conn.cursor() as cur:
        if limit:
            cur.execute(q + " LIMIT %s", params + [limit])
        else:
...
def run(conn, provider, model_id: int, prompt_row: dict, task: str,
        ...
        scheme: str = DEFAULT_SCHEME, per_city: int | None = None) -> dict:
    ...
    todo = pending_points(conn, model_id, prompt_row["prompt_id"], task, limit,
                          scheme=scheme, per_city=per_city)
    if not write:
