...
Prompt text and schema are versioned together, so changing either means a new
...
where readable text is a separate category because reading a sign is OCR, not
...
# Same city task, but the four cardinal views are composed into ONE 2x2 grid
# image (top-left north, top-right east, bottom-left south, bottom-right west).
# Portable stimulus for open-weight VLMs, incl. single-image models. Answer
# schema is identical to city_v1, so scoring is unchanged.
CITY_GRID_PROMPT_V1 = {
    "prompt_version": "city_grid_v1",
    "task": "city",
    "text": (
        "You are shown one image: a 2x2 grid of four photographs taken from "
        "the same street-level location, each facing a different cardinal "
        "direction. Top-left faces north, top-right faces east, bottom-left "
        "faces south, bottom-right faces west.\n\n"
        "Identify where in the world this location is. Give your single best "
        "guess even if you are uncertain — do not refuse and do not answer "
        "'unknown'. Estimate the coordinates of the camera position as "
        "precisely as you can.\n\n"
        "Report the visual cues that informed your guess, each classified by "
        "type, and explain your reasoning briefly. Report confidence as a "
        "number between 0 and 1."
    ),
    "output_schema": CITY_PROMPT_V1["output_schema"],
}

PROMPTS = [CITY_PROMPT_V1, CITY_GRID_PROMPT_V1, NEIGHBOURHOOD_PROMPT_V1]
