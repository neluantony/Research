...
def test_gemini_schema_strips_additional_properties_recursively():
    from inference.providers import gemini_schema

    src = next(p for p in PROMPTS if p["task"] == "city")["output_schema"]
    out = gemini_schema(src)

    def has_ap(node):
        if isinstance(node, dict):
            return "additionalProperties" in node or any(has_ap(v) for v in node.values())
        if isinstance(node, list):
            return any(has_ap(v) for v in node)
        return False

    assert has_ap(src)          # the original uses it everywhere
    assert not has_ap(out)      # the sanitized copy nowhere
    assert out["required"] == src["required"]   # everything else intact
    assert src["properties"]["cues"]["items"]["additionalProperties"] is False  # source untouched
